    ...
    <header className="w-full absolute top-0 left-0 z-50 py-8">
      <nav className="max-w-7xl mx-auto px-6 lg:px-8 flex justify-between items-center">
        <Link href="/" className="flex items-center gap-3">
          <span className="w-8 h-8 border-2 border-accent rounded-full flex items-center justify-center text-primary font-bold text-lg">O</span>
          <span className="text-xl font-bold text-primary">Oliver.</span>
        </Link>
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link key={link} href={`#${link.toLowerCase()}`} className={`text-sm font-medium transition-colors duration-300 ${link === 'Home' ? 'text-primary border-b-2 border-primary' : 'text-secondary hover:text-primary'}`}>
              {link}
            </Link>
          ))}
        </div>
